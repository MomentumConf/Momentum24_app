...
self.addEventListener("install", function (event) {
    event.waitUntil(
        caches.open(`${SCRIPT_CACHE_NAME}_${CACHE_VERSION}`).then(function (cache) {
            return cache.addAll([
                "/",
                `flutter.js?v=${CACHE_VERSION}`,
                "main.dart.js",
                `flutter_service_worker.js?v=${CACHE_VERSION}`,
            ]);
        })
    );
});
...
self.addEventListener("fetch", function (event) {
    ...
    if (event.request.url.match(/\.(jpg|jpeg|png|gif|svg|otf|ttf)$/)) {
        event.respondWith(
    ...
    if (event.request.url.match(/\.(js|css|wasm|json)$/)) {
        event.respondWith(
            caches.match(event.request).then(function (response) {
                // Cache hit - return response
                if (response) {
