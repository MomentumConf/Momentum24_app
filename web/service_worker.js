...
if (!workbox) {
    ...
    if (isLocalhost) {
        ...
        // Critical JavaScript files for app functionality
        workbox.routing.registerRoute(
            ({ url }) => url.pathname.endsWith('.js') &&
                !url.pathname.includes('service_worker.js') &&
                !url.pathname.includes('matomo.js'),
            new workbox.strategies.CacheFirst({
        ...
        // Ensure root path is handled correctly in offline mode
        const navigationHandler = workbox.precaching.createHandlerBoundToURL('/');
        workbox.routing.registerRoute(
